 ...
 * ブラウザ内で検出する」デモ。検出器は2系統:
 *   - YuNet : OpenCV 標準の軽量顔検出器 (227KB)。写真学習。近現代の写実的な顔向け。
 *             onnxruntime-web でそのまま動く。まずこれを動かす。
 *   - YOLO  : 顔コレ/浮世絵/混合データで学習する特化検出器 (ultralytics, 単一クラス)。
 *             出力 [b,5,N]=(cx,cy,w,h,score)。ONNX が用意でき次第 available=true にする。
 *
...
export type DetectorKind = "yunet" | "yolo";

export interface FaceModel {
  id: "yunet" | "kaokore" | "ukiyoe" | "ansample";
  /** セレクタのカード見出し */
  label: string;
  short: string;
  emoji: string;
